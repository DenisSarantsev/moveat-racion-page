...
(function() {
    ...
    const MIN_WIDTH = 768; // работаем только при ширине >= 768
    ...
    onReady(() => {
        if (window.innerWidth < MIN_WIDTH) {
            // Если нужно сразу показать все секции на мобильных, раскомментируй:
            // document.querySelectorAll('section').forEach((s,i)=>{ if(i===0 || !s.classList.contains(CLASS_SCROLLED)) s.classList.add(CLASS_SCROLLED); });
            return;
        }
